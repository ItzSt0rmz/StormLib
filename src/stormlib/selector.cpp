...
int autonSlot = 1;
bool stormlib::selector::isRed, stormlib::selector::isLeft,
    stormlib::selector::isSkills = false;
bool stormlib::selector::isDefault = true;

LV_IMG_DECLARE(Screen_Saver_96504D);
LV_IMG_DECLARE(Screen_Saver);

stormlib::selector::selector(int defaultAuton, const char *slot1Name,
                             const char *slot2Name, const char *slot3Name,
                             const char *slot4Name, bool is96504D) {
...
void stormlib::selector::loadSaveScreen() { lv_scr_load(saveScreen); }
...
void stormlib::selector::menuSetup() {
  exitButton = lv_btn_create(
      lv_scr_act()); // create button, lv_scr_act() is default screen object
  lv_obj_add_event_cb(exitButton, exitMenu_btn_click_action, LV_EVENT_CLICKED,
                      NULL); // set function to be called on button click
  lv_obj_add_style(exitButton, &menuStyle1,
                   LV_PART_MAIN);       // set the released style
  lv_obj_set_size(exitButton, 160, 40); // set the button size
  lv_obj_align(exitButton, LV_ALIGN_TOP_LEFT, 0,
               0); // set the position to top mid

  exitButtonLabel = lv_label_create(
      exitButton); // create label and puts it inside of the button
  lv_label_set_text(exitButtonLabel, "Exit"); // sets label text
  lv_obj_center(exitButtonLabel);

  autonsButton = lv_btn_create(
      lv_scr_act()); // create button, lv_scr_act() is default screen object
  lv_obj_add_event_cb(autonsButton, autonsMenu_btn_click_action,
                      LV_EVENT_CLICKED,
                      NULL); // set function to be called on button click
  lv_obj_add_style(autonsButton, &menuStyle1,
                   LV_PART_MAIN);         // set the released style
  lv_obj_set_size(autonsButton, 160, 40); // set the button size
  lv_obj_align(autonsButton, LV_ALIGN_TOP_LEFT, 160,
               0); // set the position to top mid

  autonsButtonLabel = lv_label_create(
      autonsButton); // create label and puts it inside of the button
  lv_label_set_text(autonsButtonLabel, "Autons"); // sets label text
  lv_obj_center(autonsButtonLabel);

  skillsButton = lv_btn_create(
      lv_scr_act()); // create button, lv_scr_act() is default screen object
  lv_obj_add_event_cb(skillsButton, skillsMenu_btn_click_action,
                      LV_EVENT_CLICKED,
                      NULL); // set function to be called on button click
  lv_obj_add_style(skillsButton, &menuStyle1,
                   LV_PART_MAIN);         // set the released style
  lv_obj_set_size(skillsButton, 160, 40); // set the button size
  lv_obj_align(skillsButton, LV_ALIGN_TOP_LEFT, 320,
               0); // set the position to top mid

  skillsButtonLabel = lv_label_create(
      skillsButton); // create label and puts it inside of the button
...
void stormlib::selector::initialize() {
  ...
  lv_style_set_bg_grad_color(&myButtonStyleREL,
  ...
  lv_obj_add_event_cb(redButton, red_btn_click_action, LV_EVENT_CLICKED,
                      NULL); // set click event
  lv_obj_add_style(
      redButton, &redStyle,
      LV_PART_MAIN); // set the style for released and pressed state
  ...
  lv_obj_add_event_cb(blueButton, blue_btn_click_action, LV_EVENT_CLICKED,
                      NULL); // set click event
  lv_obj_add_style(
      blueButton, &blueStyle,
      LV_PART_MAIN); // set the style for released and pressed state
  ...
  lv_obj_add_event_cb(leftButton, left_btn_click_action, LV_EVENT_CLICKED,
                      NULL); // set click event
  ...
  lv_obj_add_event_cb(rightButton, right_btn_click_action, LV_EVENT_CLICKED,
                      NULL); // set click event
  ...
  lv_obj_add_event_cb(backButton1, back_button_click_action1, LV_EVENT_CLICKED,
                      NULL);                               // set click event
  ...
  lv_obj_add_event_cb(option1Button, opt1_btn_click_action, LV_EVENT_CLICKED,
                      NULL); // set click event
  lv_obj_add_style(option1Button, &blackStyle,
                   LV_PART_MAIN);                        // set released style
  lv_obj_set_size(option1Button, 240, 100);              // set button size
  lv_obj_align(option1Button, LV_ALIGN_TOP_LEFT, 0, 40); // set position

  option1ButtonLabel =
      lv_label_create(option1Button); // create label inside button
  ...
  lv_obj_add_event_cb(option2Button, opt2_btn_click_action, LV_EVENT_CLICKED,
                      NULL); // set click event
  lv_obj_add_style(option2Button, &blackStyle,
                   LV_PART_MAIN);                          // set released style
  lv_obj_set_size(option2Button, 240, 100);                // set button size
  lv_obj_align(option2Button, LV_ALIGN_TOP_LEFT, 240, 40); // set position

  option2ButtonLabel =
      lv_label_create(option2Button); // create label inside button
  ...
  lv_obj_add_event_cb(option3Button, opt3_btn_click_action, LV_EVENT_CLICKED,
                      NULL); // set click event
  lv_obj_add_style(option3Button, &blackStyle,
                   LV_PART_MAIN);                         // set released style
  lv_obj_set_size(option3Button, 240, 100);               // set button size
  lv_obj_align(option3Button, LV_ALIGN_TOP_LEFT, 0, 140); // set position

  option3ButtonLabel =
      lv_label_create(option3Button); // create label inside button
  ...
  lv_obj_add_event_cb(option4Button, opt4_btn_click_action, LV_EVENT_CLICKED,
                      NULL); // set click event
  lv_obj_add_style(option4Button, &blackStyle,
                   LV_PART_MAIN);           // set released style
  lv_obj_set_size(option4Button, 240, 100); // set button size
  lv_obj_align(option4Button, LV_ALIGN_TOP_LEFT, 240, 140); // set position

  option4ButtonLabel =
      lv_label_create(option4Button); // create label inside button
  ...
  lv_obj_add_event_cb(backButton2, back_button_click_action2, LV_EVENT_CLICKED,
                      NULL); // set click event
  lv_obj_add_style(backButton2, &backStyle,
                   LV_PART_MAIN);        // set released style and pressed style
  ...
  lv_obj_add_event_cb(skills1Button, skl1_btn_click_action, LV_EVENT_CLICKED,
                      NULL);
  lv_obj_add_style(skills1Button, &blackStyle, LV_PART_MAIN);
  ...
  lv_obj_add_event_cb(skills2Button, skl2_btn_click_action, LV_EVENT_CLICKED,
                      NULL);
  lv_obj_add_style(skills2Button, &blackStyle, LV_PART_MAIN);
  ...
  lv_obj_add_event_cb(skills3Button, skl3_btn_click_action, LV_EVENT_CLICKED,
                      NULL);
  lv_obj_add_style(skills3Button, &blackStyle, LV_PART_MAIN);
  ...
  lv_obj_add_event_cb(skills4Button, skl4_btn_click_action, LV_EVENT_CLICKED,
                      NULL);
  lv_obj_add_style(skills4Button, &blackStyle, LV_PART_MAIN);
  ...
  lv_obj_align(saveExit, LV_ALIGN_TOP_LEFT, 0, 0); // set the position to top
                                                   // mid
...
int stormlib::selector::getAuton() {
  ...
  if (!isSkills) {
    if (isRed) {
      if (isLeft) {
        if (autonSlot == 1)
          return stormlib::selector::E_RED_LEFT_1;
        if (autonSlot == 2)
          return stormlib::selector::E_RED_LEFT_2;
        if (autonSlot == 3)
          return stormlib::selector::E_RED_LEFT_3;
        if (autonSlot == 4)
          return stormlib::selector::E_RED_LEFT_4;
      } else {
        if (autonSlot == 1)
          return stormlib::selector::E_RED_RIGHT_1;
        if (autonSlot == 2)
          return stormlib::selector::E_RED_RIGHT_2;
        if (autonSlot == 3)
          return stormlib::selector::E_RED_RIGHT_3;
        if (autonSlot == 4)
          return stormlib::selector::E_RED_RIGHT_4;
      }
    } else {
      if (isLeft) {
        if (autonSlot == 1)
          return stormlib::selector::E_BLUE_LEFT_1;
        if (autonSlot == 2)
          return stormlib::selector::E_BLUE_LEFT_2;
        if (autonSlot == 3)
          return stormlib::selector::E_BLUE_LEFT_3;
        if (autonSlot == 4)
          return stormlib::selector::E_BLUE_LEFT_4;
      } else {
        if (autonSlot == 1)
          return stormlib::selector::E_BLUE_RIGHT_1;
        if (autonSlot == 2)
          return stormlib::selector::E_BLUE_RIGHT_2;
        if (autonSlot == 3)
          return stormlib::selector::E_BLUE_RIGHT_3;
        if (autonSlot == 4)
          return stormlib::selector::E_BLUE_RIGHT_4;
      }
    }
  } else {
    if (autonSlot == 1)
      return stormlib::selector::E_SKILLS_1;
    if (autonSlot == 2)
      return stormlib::selector::E_SKILLS_2;
    if (autonSlot == 3)
      return stormlib::selector::E_SKILLS_3;
    if (autonSlot == 4)
      return stormlib::selector::E_SKILLS_4;
  }

  return 1;
}

bool stormlib::selector::isAllianceRed() { return isRed; }
